...
public class AugmentedDrops implements Listener {
    private final BadlandsCaves plugin;
    private final NamespacedKey key;

    public AugmentedDrops(BadlandsCaves plugin) {
        this.plugin = plugin;
        key = new NamespacedKey(plugin, "augmented");
    }
    ...
    public void killMob (EntityDeathEvent event) {
        ...
        if (entity.getPersistentDataContainer().has(key, PersistentDataType.BYTE)) {
            ...
            if (result != null && result == (byte) 1) {
                final int multiplier = plugin.getOptionsConfig().getInt("hardmode_values.augmented_exp_multiplier");
