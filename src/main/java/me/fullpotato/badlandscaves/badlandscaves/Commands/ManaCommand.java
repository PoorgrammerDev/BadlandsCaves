            ...
            if (player.isOp()) {
                if (args.length == 0) {
                    ...
                    if (args.length < 2) {
                        ...
                        for (Player targets : Bukkit.getOnlinePlayers()) {
                            if (args[1].equalsIgnoreCase(targets.getDisplayName()) || args[1].equalsIgnoreCase(targets.getName()) || args[1].equalsIgnoreCase(targets.getUniqueId().toString())) {
                                player.sendMessage(ChatColor.GOLD + "The Mana count of " + ChatColor.RED + targets.getDisplayName() + ChatColor.GOLD + " is " + ChatColor.RED + targets.getMetadata("Mana").get(0).asDouble() + ChatColor.GOLD + ".");
                                return true;
                            }
                        }
                    }
                    ...
                    if (args.length < 2) {
                        ...
                        player.sendMessage(ChatColor.RED + "Please specify a Mana value to set to.");
                        return true;
                    } else {
                        for (Player targets : Bukkit.getOnlinePlayers()) {
                            if (args[1].equalsIgnoreCase(targets.getDisplayName()) || args[1].equalsIgnoreCase(targets.getName()) || args[1].equalsIgnoreCase(targets.getUniqueId().toString())) {
                                try {
                                    ...
                                    player.sendMessage(ChatColor.GOLD + "The Mana count of " + ChatColor.RED + targets.getDisplayName() + ChatColor.GOLD + " has been set to " + ChatColor.RED + change + ChatColor.GOLD + ".");
                                    return true;
                                }
                                catch (NumberFormatException e) {
                                    player.sendMessage(ChatColor.RED + "You must set the Mana count to a double!");
                                    return true;
                                }
                            }
                        }
