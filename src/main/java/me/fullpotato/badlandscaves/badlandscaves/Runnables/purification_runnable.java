...
public class purification_runnable extends BukkitRunnable {
    ...
    public purification_runnable (BadlandsCaves bcav, Inventory inv, Location loc) {
        plugin = bcav;
        inventory = inv;
        location = loc;
    }
    ...
    @Override
    public void run() {
        Block cauldron_block = location.getBlock();
        String block_desc = cauldron_block.getBlockData().getAsString();
